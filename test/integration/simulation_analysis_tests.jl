...
@testset "Steady state diagnostics" begin
    ...
    sol_s = ThesisProject.solve(model_s, method_s, cfg_s; rng = derive_solver_rng(cfg_s, "EGM"))
...
@testset "Panel simulation" begin
    ...
    model, method = build_model_and_method(cfg)
    panel = Sim.simulate_panel(model, method, cfg; N = 10, T = 8, master_rng = cfg.random.master_rng)
    @test size(panel.assets) == (10, 8)
    @test size(panel.consumption) == (10, 8)
    @test length(panel.seeds) == 10
    @test panel.diagnostics.master_seed == ThesisProject.Determinism.master_seed(cfg.random.master_rng)
end
