...
runid = get(cfg, "runid", "det_egm_pchip")

# --- Solver
sol = EGMSolver.solve_simple_egm(p, agrid; tol=1e-10, maxit=5000, verbose=true, relax = 0.3, interp_kind=EGMSolver.MonotoneCubicInterp())
...
CSV.write(joinpath("runs",runid,"deterministic_egm_pchip.csv"), df)
