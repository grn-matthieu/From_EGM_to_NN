# Documentation index

This folder holds supporting documentation, figures, and notes used while developing and running the experiments in this repository.

Key locations

- `docs/figures/` — generated figures used in reports and the thesis.
- `docs/plan/` — notes, plans, and small docs (e.g. `repo_inventory.md`, `repo_tree.md`).

How to use

- Read the top-level `README.md` for quick start instructions.
- To reproduce experiments, inspect the YAML configs in `config/` and the drivers in `scripts/experiments`.

If you add new figures or experiment outputs, add a short README inside the new folder explaining how to re-generate them.
