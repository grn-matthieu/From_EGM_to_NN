...
function log_row!(
    ...
    open(lg.path, lg.header_written[] ? "a" : "w") do io
        if !lg.header_written[]
            println(io, "timestamp,epoch,step,split,loss,grad_norm,lr")
...
"""
function _step!(state::NNState, x, y; clip_norm = nothing)
...
function train!(state::NNState, data, cfg; val_data = nothing)
    ...
    for epoch = 1:epochs
        ...
        for (x, y) in data
            step += 1
            n_steps += 1
            state, loss, gnorm, lr = _step!(state, x, y; clip_norm = clip_norm)
            loss_accum += loss
