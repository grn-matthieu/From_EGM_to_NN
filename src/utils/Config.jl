...
Performs normalization of a few benign inputs (e.g., symbols/strings, beta → β) and
...
function validate_config(cfg::AbstractDict)
    ...
    params = cfg[:params]
    # normalize aliases: beta → β, s → σ (as used throughout kernels)
    beta_sym = Symbol("β")
    if haskey(params, :beta) && !haskey(params, beta_sym)
        params[beta_sym] = params[:beta]
    end
    sigma_sym = Symbol("σ")
    if haskey(params, :s) && !haskey(params, sigma_sym)
        params[sigma_sym] = params[:s]
    end
    for req in (beta_sym, sigma_sym, :r, :y)
        haskey(params, req) || error("params.$(req) missing")
        (params[req] isa Real) ||
            error("params.$(req) must be a number; got $(typeof(params[req]))")
    end
    (0 < params[beta_sym] < 1) ||
        error("params.β must satisfy 0 < β < 1; got $(params[beta_sym])")
    # σ > 0, allow ~1 for log utility
    (params[sigma_sym] > 0) || error("params.σ must be > 0; got $(params[sigma_sym])")
    ...
    if haskey(cfg, :shocks) &&
       ...
       get(cfg[:shocks], :active, false)
        ...
        # keys and ranges
        # accept ascii alias for rho_shock
        rho_key =
            haskey(sh, Symbol("ρ_shock")) ? Symbol("ρ_shock") :
            (haskey(sh, :rho_shock) ? :rho_shock : nothing)
        rho =
            rho_key === nothing ?
            error("shocks.ρ_shock (or rho_shock) missing when shocks.active = true") :
            sh[rho_key]
