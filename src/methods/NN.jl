...
function build_nn_method(cfg::NamedTuple)
    ...
    return NNMethod((
        ...
        resample_every = cfg_get(nn_cfg, :resample_every, 1),
...
function solve(
    ...
    metadata = Dict{Symbol,Any}(
        ...
        :N_history => hasproperty(sol, :N_history) ? sol.N_history : Int[],
        :v_h_history => hasproperty(sol, :v_h_history) ? sol.v_h_history : Float64[],
        :opts => sol.opts,
    )
